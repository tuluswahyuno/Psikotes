        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}" class="form-input" required placeholder="Contoh: Paket Simulasi Nasional 1">
                            @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="form-label" for="duration_minutes">Durasi Total (menit) <span class="text-red-500">*</span></label>
                            <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', 100) }}" class="form-input" required min="1">
                        </div>
                        <div>
                            <label class="form-label" for="description">Deskripsi</label>
                            <textarea name="description" id="description" rows="4" class="form-input" placeholder="Keterangan untuk peserta...">{{ old('description') }}</textarea>
                        </div>
                        <label class="flex items-center gap-2 cursor-pointer mt-2">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="lg:col-span-2 space-y-6">
                                ...
                                <div>
                                    ...
                                    <select name="twk_test_id" id="twk_test_id" class="form-input" required>
                                        ...
                                        @foreach($tests as $test)
                                            <option value="{{ $test->id }}" {{ old('twk_test_id') == $test->id ? 'selected' : '' }}>{{ $test->title }} ({{ $test->questions_count ?? 0 }} soal)</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="form-label text-blue-700" for="twk_passing_grade">Passing Grade <span class="text-red-500">*</span></label>
                                    <input type="number" name="twk_passing_grade" id="twk_passing_grade" value="{{ old('twk_passing_grade', 65) }}" class="form-input" required>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="tiu_test_id" id="tiu_test_id" class="form-input" required>
                                        <option value="">-- Pilih Tes --</option>
                                        @foreach($tests as $test)
                                            <option value="{{ $test->id }}" {{ old('tiu_test_id') == $test->id ? 'selected' : '' }}>{{ $test->title }} ({{ $test->questions_count ?? 0 }} soal)</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="form-label text-amber-700" for="tiu_passing_grade">Passing Grade <span class="text-red-500">*</span></label>
                                    <input type="number" name="tiu_passing_grade" id="tiu_passing_grade" value="{{ old('tiu_passing_grade', 80) }}" class="form-input" required>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="tkp_test_id" id="tkp_test_id" class="form-input" required>
                                        <option value="">-- Pilih Tes --</option>
                                        @foreach($tests as $test)
                                            <option value="{{ $test->id }}" {{ old('tkp_test_id') == $test->id ? 'selected' : '' }}>{{ $test->title }} ({{ $test->questions_count ?? 0 }} soal)</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="form-label text-purple-700" for="tkp_passing_grade">Passing Grade <span class="text-red-500">*</span></label>
                                    <input type="number" name="tkp_passing_grade" id="tkp_passing_grade" value="{{ old('tkp_passing_grade', 166) }}" class="form-input" required>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center gap-3 pt-4">
                    <button type="submit" class="btn-primary px-8">Simpan Paket SKD</button>
                    <a href="{{ route('admin.skd-packages.index') }}" class="btn-secondary">Batal</a>
                </div>
            </div>
        </div>
    </form>
